...
module.exports = {
    ...
    entry: {
        //webpack打包多入口配置
        // 代码分离(多入口)方式1 https://www.bilibili.com/video/BV1YU4y1g745?p=41
        // index: './src/index.js',
        // another: './src/another-module.js'
        //缺点:会为每个入口都打包引用的相同的第三方库，导致打包后代码文件过大且重复(解决方式:方式2)

        // 代码分离(多入口)方式2 https://www.bilibili.com/video/BV1YU4y1g745?p=42
        // index: {
        //     import: './src/index.js',
        //     dependOn: 'shared'
        // },
        ...
        //     dependOn: 'shared'
        // },
        // shared:'lodash' //当上面的index和another有用lodash库(或他们引用了其他第三方库或公共文件，可在此指定)时，就会抽离
                        //出来并且取名为叫shared这样的一个chunk；即将index和another都依赖的第三方库抽离成单独文件让他们去依赖，
                        //而不是为index和another分别打包同样的第三库；这就避免了打包的重复，减小了打包后文件的大小


        // 代码分离(多入口)方式2-1 https://www.bilibili.com/video/BV1YU4y1g745?p=42  4m:10s
        //配合下面的splitChunks属性配置
        // index: './src/index.js',
        // another: './src/another-module.js'

        // 代码分离(多入口)方式3 https://www.bilibili.com/video/BV1YU4y1g745?p=43
        //使用import动态导入
        index: './src/index.js',
    },
    ...
    output: {//文件打包后输出的路径以及名称等
        ...
        filename: "[name].bundle.js",//有多个入口文件时，需要对应多个输出文件，否则会报错;[name]对应的是打包入口配置的key
    ...
    optimization: {
        ...
        // 代码分离(多入口)方式2-1 https://www.bilibili.com/video/BV1YU4y1g745?p=42  4m-10s
        // 方式2-1的配置同样可以将公共的文件抽离出来形成一个单独的chunks
        splitChunks: {
            chunks: "all"
        }
    }
